...
impl Client {
    ...
    /// # Returns
    ///
    /// Returns the address associated with the vault and asset.
    ///
    /// # Errors
    ///
    /// This method can fail if:
    ...
    pub fn address(&self, vault: &str, asset: impl AsRef<str> + Display + Debug) -> Result<String> {
    ...
    /// - [`Option<String>`] the blockchain tx hash
    ///
    /// # Errors
    ///
    /// This method can fail if:
    ...
    /// - The response cannot be parsed
    pub fn get_tx(&self, txid: &str) -> Result<(TransactionResponse, Option<String>)> {
    ...
    ) -> Result<(TransactionResponse, Option<String>)> {
